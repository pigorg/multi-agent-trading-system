...
def pipeline(
    ...
    show_basket: bool = typer.Option(
        False, "--show-basket", help="Print the configured screener basket and exit (no run, no cost)."
    ),
    yes: bool = typer.Option(
        False, "--yes", "-y", help="Skip the confirmation prompt (Agent 2/3 make paid LLM calls)."
    ),
):
    """Run the 3-agent screener -> deep-analysis -> allocator pipeline with live progress,
    emailing a report after each stage."""
    import datetime as _dt

    from tradingagents.pipeline import charts as pipeline_charts
    from tradingagents.pipeline import config as pipeline_config
    from tradingagents.pipeline.allocator import run_allocator
    from tradingagents.pipeline.deep_analysis import run_deep_analysis
    from tradingagents.pipeline.emailer import send_report
    from tradingagents.pipeline.orchestrator import (
        format_final_report,
        format_finalists_report,
        format_screener_report,
    )
    from tradingagents.pipeline.screener import run_screener

    trade_date = date or _dt.date.today().isoformat()
    basket = pipeline_config.PIPELINE_BASKET

    console.print(Panel.fit("[bold]TradingAgents — Pipeline a 3 agenti[/bold]", border_style="cyan"))
    console.print(f"[dim]Paniere ({len(basket)} titoli):[/dim] {', '.join(basket)}")
    console.print(
        f"[dim]Destinatario email:[/dim] {pipeline_config.REPORT_EMAIL_TO}"
        + ("" if pipeline_config.SMTP_HOST else "  [yellow](SMTP non configurato: i report verranno stampati qui)[/yellow]")
    )
    ...
    if not yes:
        proceed = typer.confirm(
            "\nGli agenti 2 e 3 fanno chiamate LLM a pagamento sui titoli selezionati. Procedere?",
            default=True,
        )
        if not proceed:
    ...
    # --- Agent 1: screener ---
    console.rule("[bold cyan]Agente 1 — Screener quantitativo")
    with console.status("[cyan]Scarico dati e filtro il paniere...[/cyan]"):
        screened = run_screener()

    if not screened:
        console.print("[red]Nessun titolo ha superato i filtri oggi.[/red]")
        send_report(
    ...
    t1 = Table(title=f"{len(screened)} titoli selezionati")
    t1.add_column("Ticker")
    t1.add_column("Score", justify="right")
    t1.add_column("Prezzo", justify="right")
    t1.add_column("Drawdown", justify="right")
    t1.add_column("Motivi")
    for r in screened:
        t1.add_row(r.ticker, f"{r.score:.2f}", f"{r.price:.2f}", f"{r.drawdown:.1%}", "; ".join(r.reasons))
    console.print(t1)

    send_report(*format_screener_report(screened, trade_date))
    console.print("[green]✓ Report Agente 1 inviato/stampato.[/green]\n")

    # --- Agent 2: deep analysis ---
    console.rule("[bold cyan]Agente 2 — Analisi approfondita (parallela)")
    console.print(f"[dim]Fino a {pipeline_config.DEEP_ANALYSIS_WORKERS} titoli in parallelo...[/dim]")

    def _on_result(r):
        color = "green" if r.rating in ("Buy", "Overweight") else ("yellow" if r.rating == "Hold" else "red")
        console.print(f"  [{color}]{r.ticker}[/{color}] completato — rating: {r.rating}")

    finalists = run_deep_analysis(
        [r.ticker for r in screened], trade_date=trade_date, on_result=_on_result
    )

    if not finalists:
        console.print("[red]Nessun finalista prodotto.[/red]")
        send_report(
    ...
    t2 = Table(title=f"{len(finalists)} finalisti")
    t2.add_column("Ticker")
    t2.add_column("Rating")
    t2.add_column("Report")
    for r in finalists:
        t2.add_row(r.ticker, r.rating, r.report_path or "n/d")
    console.print(t2)

    send_report(*format_finalists_report(finalists, trade_date))
    console.print("[green]✓ Report Agente 2 inviato/stampato.[/green]\n")

    # --- Agent 3: allocator ---
    console.rule("[bold cyan]Agente 3 — Allocazione finale")
    with console.status("[cyan]Confronto i finalisti e genero i grafici...[/cyan]"):
        allocation = run_allocator(finalists)
        chart_paths = pipeline_charts.build_charts([r.ticker for r in finalists])

    console.print(
        Panel(
            f"[bold]{allocation.winner}[/bold]\n\n{allocation.reasoning}",
            title="Decisione finale",
            border_style="green",
        )
    )
    if chart_paths:
        console.print(f"[dim]Grafici generati:[/dim] {', '.join(chart_paths)}")

    subject, body = format_final_report(allocation, trade_date)
    send_report(subject, body, attachments=chart_paths)
    console.print("[green]✓ Report finale inviato/stampato.[/green]")
